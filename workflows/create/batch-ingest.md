...
2. **Dispatch the parallel subagents under the protocol.** Run [parallel subagent protocol](../_shared/procedures/parallel-subagent-protocol.md) in full, then return here and continue with step 3. The fragment owns: per-agent scope boundaries, the canonical coordinator-only file enumeration, the report-not-edit instruction, and the dispatch contract. Each subagent's task is "run [ingest](ingest.md) end-to-end on paper X" — the protocol fragment's invariants ensure the per-paper work stays isolated.
3. **Spot-check the agent output.** Run [spot check agent output](../_shared/procedures/spot-check-agent-output.md), then return here and continue with step 4. Each per-paper ingest already runs verify-frontmatter-completeness, so the spot-check should verify this was done rather than re-running it. If the spot check escalates (2+ issues across the sample), pause and run `workflows/audit/verification.md` in full before consolidation.
...
5. Run the [Verification](../audit/verification.md) workflow on all subagent output. The spot check in step 3 is the lightweight first pass; verification is the heavyweight per-page review.

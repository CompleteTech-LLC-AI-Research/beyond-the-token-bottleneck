...
1. **Sample, do not exhaustively review.** Pick 2–3 pages from the agent output (or one per agent if there are few). Exhaustive review is the job of `workflows/audit/verification.md`; this is the catch-the-obvious-stuff pass.
...
6. **Decide whether to escalate.** If the spot check finds two or more issues across the sample, the parallel phase's output is suspect at scale and the workflow should escalate to a full verification pass via `workflows/audit/verification.md` before the work is consolidated.

## Invariants

- This is a sampling check, not an exhaustive review. Do not let it expand into a per-page audit — that escalation belongs to `workflows/audit/verification.md`.
...
- Numbers, links, and frontmatter are the three load-bearing categories. Other concerns (prose quality, redundancy, depth) belong to `workflows/audit/review.md` and `workflows/enrich/enrich.md`, not this spot check.
...
- `workflows/audit/enrichment-audit.md` (Phase 4 step 4, replacing the inlined "Spot-check agent output" line)
- `workflows/audit/verification.md` (as the entry-point lightweight check before the heavier per-page review subagents are dispatched)
- `workflows/create/batch-ingest.md` (after parallel ingest phase, before consolidation)
- `workflows/audit/moc-gap-analysis.md` (after parallel MOC-creation subagents complete)
